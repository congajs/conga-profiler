...
export default Vue.extend({
                ...
                <div class="container" slot="hero-foot">
                    ...
                    <div class="tabs is-boxed">
                        <ul>
                            ...
                            <li v-if="hasClientProfiler">
                                <router-link :to="{name: 'client-profiler'}">
                                    Client Sessions
                                </router-link>
                            </li>
                        </ul>
                    ...
                    <table class="table is-striped is-narrow is-fullwidth" style="font-size: 11px;">
                        ...
                        <tbody>
                            <tr v-for="data in requests" v-on:click="open(data.profilerId)">
                                ...
                                <td>
                                    {{ data.createdAt | moment('H:mm:ss') }}.{{ data.createdAt | moment('SSS') }}
                                </td>
                                <td>
                                    {{ (data.requestTime / 1000 / 1000).toFixed(3) }} seconds
                                </td>
                            </tr>
                        </tbody>
                    </table>
    ...
    data: function() {
        return {
            ...
            hasClientProfiler: false
        };
    },

    created: function() {
        HttpService.get(this.$http, this.$router, '_conga/profiler/requests').then((response) => {
            this.requests = response.body.requests;
            this.hasClientProfiler = response.body.hasClientProfiler;
        }, (response) => {
