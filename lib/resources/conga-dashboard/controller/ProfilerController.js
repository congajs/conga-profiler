...
module.exports = class ProfilerController extends Controller {
    ...
    requests(req, res) {
        ...
        return profilerRequest.getRequestsBy({}, {sort: {createdAt: -1}, limit: 10}).then(requests => {
            const data = {
                token,
                requests,
                hasClientProfiler: this.container.get('bundle.finder').has('@conga/framework-profiler-client'),
                navigation: requests.length === 0 ? [] : navigation(Object.keys(requests[0].collectedData), profilerDashboard)
            };
            return Promise.resolve(data);
        }).catch(err => res.return({error: err.stack || err}, 500));
    }

    /**
     * @Route("/request/:profilerId", methods=["GET"])
     */
    request(req, res) {
        const profilerRequest = this.container.get('profiler.request');
        const profilerDashboard = this.container.get('profiler.dashboard');
    ...
    /**
     * @Route("/request/:profilerId/:collectorName", methods=["GET"])
     */
