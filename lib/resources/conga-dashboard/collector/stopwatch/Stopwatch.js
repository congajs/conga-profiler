...
TimelineGraph.CANVAS_COLORS = [
    '#007DF9', '#B16DF7', '#FF5BD9', '#3CB371', '#832C2C', '#FFB431', '#1DCCF9', '#FF0042', '#CB813A', '#5D05A9', '#96831b', '#000BFF', '#108181'
];
...
TimelineGraph.prototype = {
    ...
    calcWidth: function(startedAt, finishedAt, timeScale) {
        return this.calcLeft(finishedAt, timeScale) - this.calcLeft(startedAt, timeScale)
    },

    calcBarWidth: function(startedAt, finishedAt, timeScale) {
        if (!this.hasCanvas) {
            this._initCanvas();
        }
        return Math.max(this.canvas.barWidthMin, this.calcWidth(startedAt, finishedAt, timeScale));
    },
    ...
    _renderSectionBackgrounds: function(timeScale, heightScale) {
        ...
        this.canvas.svg.append('g').attr('class', 'section-background').selectAll('rect')
            ...
            .attr('width', function(d) {
                var width = self.calcWidth(d.firstPeriod.startedAt, d.lastPeriod.finishedAt, timeScale);
                var barWidth = self.calcBarWidth(d.firstPeriod.startedAt, d.lastPeriod.finishedAt, timeScale);
                return barWidth + (barWidth - width);
            });
        ...
        this.canvas.svg
            ...
            .each(function(d) {
                ...
                var i,
                    ...
                    timeStr,
                    mbStr = '',
                    name = d.isSection ? d.section : d.name,
                    title = name;

                if (d.periods && d.periods.length !== 0) {
                    title += ' @ ' + self.config.timeFormat(d.firstPeriod.startedAt);
                    ...
                    label = name + ' ' + timeStr + ' / ' + mbStr;

                    if (d.periods.length !== 1) {
                        title += '\n';
                        for (i = 0; i < d.periods.length; i++) {
                            title += '\n~ ' + (d.periods[i].memory.rss / 1024 / 1024).toFixed(3) + ' MB';
                            title += ' @ ' + self.config.timeFormat(d.periods[i].startedAt);
                        }
                    }
                }
                ...
                d3.select(this).selectAll('rect').data([d]).enter()
                    ...
                    .attr('x', function(d) {
                        var width = self.calcWidth(d.firstPeriod.startedAt, d.lastPeriod.finishedAt, timeScale);
                        var barWidth = self.calcBarWidth(d.firstPeriod.startedAt, d.lastPeriod.finishedAt, timeScale);
                        return self.calcLeft(d.firstPeriod.startedAt, timeScale) - (barWidth - width);
                    })
                    .attr('y',function(d) { return self.calcBarTop(d.idx, heightScale); })
                    .attr('height', self.canvas.barHeight)
                    .attr('width', function(d) {
                        return self.calcBarWidth(d.firstPeriod.startedAt, d.lastPeriod.finishedAt, timeScale);
                    });
    ...
    _offsetHeight: function(add) {
        ...
        // TODO: the rotation on the time ticker isn't being calculated correctly and so it gets cut off, so the 150 is to make sure that doesn't happen for now
        this.canvas.svg.attr('height', this.canvas.fnOuterHeight() + 150);
        return this;
    }
};
    ...
    for (i = 0; i < len; i++) {
        ...
        for (x = 0; x < xlen; x++) {
            ...
            if (event.periods.length !== 0) {
                if (!(event.id in dataset) && event.name) {
