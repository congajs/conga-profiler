...
class Request {
    constructor() {
        ...
        /**
         * @Bass:Field(type="Object", name="firewall")
         * @Rest:Attribute
         */
        this.firewall = null;
    ...
    /**
     * Handle some special encoding
     * @Bass:PrePersist
     */
    onPrePersist(evt, cb) {

        const document = evt.document;

        if (!(document.request instanceof Object)) {
            cb();
            return;
        }

        // NOTE: we stringify for mongo to avoid errors with dots in the keys
        // TODO: make this configurable some how

        if (document.request.query instanceof Object) {
            try {
                document.request.query = JSON.stringify(document.request.query);
            } catch (e) {
    ...
    onPreHydrate(evt, cb) {
        ...
        if (document.request.query instanceof Object) {
            try {
                document.request.query = JSON.parse(document.request.query);
            } catch (e) {
                console.error(e.stack || e);
            }
        }
