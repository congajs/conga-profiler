...
// framework libs
const { Authenticator, Error } = require('@conga/framework-security');
...
class ProfilerAuthenticator extends Authenticator.AbstractAuthenticator {
                ...
                if (this.container.has(serviceId)) {
                    ...
                    if (service instanceof Authenticator.AbstractAuthenticator) {
                        ...
                        throw new Error.ConfigurationError(
                            ...
                            '@conga/framework-security:security/authenticator/AbstractAuthenticator');
                    }
                } else {
                    throw new Error.ConfigurationError(
                        'Invalid service id referenced for a profiler authenticator.');
                }
            }
        }
    ...
    refreshToken(token, provider, realm) {
        const authenticator = this._getAuthenticator();
        if (authenticator instanceof Authenticator.AbstractAuthenticator) {
            return authenticator.refreshToken(token, provider, realm);
        }
        return super.refreshToken(token, provider, realm);
    }
