    ...
    matches(request) {
        ...
        for (name in config) {
            ...
            if (typeof matcher === 'string') {
                // we have to check for this on each request because the service may live in the
                // request scope and so we cannot gather them on compile

                // NOTE: if performance becomes an issue, we can gather the sid's on compile and
                // perform the regexp there
                let sid = matcher.replace(/^@/, '');
                if (this.container.has(sid)) {
                    matcher = this.container.get(sid);
                    if (!(matcher instanceof RequestMatcherInterface)) {
                        throw new LogicError(
                            'Invalid matcher configuration provided as, "' + name + '".');
                    }
                    return matcher.matches(request);
                }
            } else if (matcher instanceof RequestMatcherInterface) {
                return matcher.matches(request);
