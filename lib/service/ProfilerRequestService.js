...
class ProfilerRequestService {
    ...
    startRequest(request, response) {
        ...
        const document = manager.createDocument('Request', {
            ...
            authToken: securityContext && securityContext.getAuthToken(),
            startedAt: microtime,
            createdAt: new Date()
        });

        // the first request stat is always the stat gathered prior to (or at the same of) the request
        document.stats = [ this.container.get('profiler.stat').getCurrentStat() ];
    ...
    stopRequest(request, response, collectors = []) {
        ...
        return this.collectDataForRequest(collectors, request, response, document).then(collected => {
            ...
            document.collectedData = collected;
    ...
    getChildRequests(request, sort = {createdAt: -1}) {
        const referer = request.prototype + '://' + request.headers.host + request.originalUrl;
        return this.getRequestsBy({request: {headers: { referer }}}, { sort });
    }
    ...
    collectDataForRequest(dataCollectors, request, response, document = null) {
        ...
        // stopwatch event
        const stopwatch = this.container.get('profiler.stopwatch').request(request).start('profiler.collector', 'profiler');
        // the collected data
        const collected = {};
        // execute all collectors asynchronously
        return Promise.all(dataCollectors.map(collector => {
            stopwatch.lap();
            if (!(collector instanceof DataCollectorInterface)) {
                throw new ProfilerError(
                    ...
                    'conga-profiler:profiler/DataCollectorInterface');
            }
        ...
        .catch(err => stopwatch.stop() && console.error(err.stack || err));
    }
}
